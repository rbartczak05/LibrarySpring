...
class ClientEndpointTest {
    ...
    private Client buildClient(UUID id, String firstName, String lastName) {
    ...
    void getAllClients_shouldReturnList() {
        when(clientUseCase.findAllClients()).thenReturn(List.of(
                buildClient(client1Id, "Jan", "Kowalski")
        ));

        mockClient.sendRequest(withPayload(new StringSource(
                        "<GetAllClientsRequest xmlns=\"" + NS + "\"/>")))
                .andExpect(noFault())
                .andExpect(xpath("//ns:client[1]/ns:id", NS_MAP).evaluatesTo(client1Id.toString()))
                .andExpect(xpath("//ns:client[1]/ns:firstName", NS_MAP).evaluatesTo("Jan"));
    }

    @Test
    void getClientById_shouldReturnClient() {
        when(clientUseCase.findClientById(client1Id)).thenReturn(buildClient(client1Id, "Jan", "Kowalski"));

        mockClient.sendRequest(withPayload(new StringSource(
                        "<GetClientByIdRequest xmlns=\"" + NS + "\"><id>" + client1Id + "</id></GetClientByIdRequest>")))
                .andExpect(noFault())
